...
#include <string.h>
#include <iostream>
#include "hash-it-out/fifo_evictor.hh"
#define CATCH_CONFIG_MAIN
#include "catch.hpp"

FifoEvictor* test_evictor = new FifoEvictor();
Cache test_cache = Cache(1024, 0.75, test_evictor);

TEST_CASE( "test_fifo_evictor", "[placeholder]" ) {    
    ...
    s = 499;
    REQUIRE(test_cache.get("key1", s) != nullptr);
    REQUIRE(strcmp(charr1, test_cache.get("key1", s)) == 0);
    REQUIRE(test_cache.space_used() == 499);
    ...
    REQUIRE(test_cache.get("key1", s) != nullptr);
    s = 297;
    REQUIRE(test_cache.get("key2", s) != nullptr);
    REQUIRE(test_cache.space_used() == 499+297);
    delete test_evictor;
}
